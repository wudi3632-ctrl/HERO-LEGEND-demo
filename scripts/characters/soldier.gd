...
# ---------- ChaseState ----------
class SoldierChaseState extends State:
	func enter() -> void:
		enemy.has_dealt_damage = false

	func update(_delta: float) -> void:
		if not enemy.player:
			enemy.transition_to("SoldierPatrolState")
			return
		...
		if dist < enemy.ATTACK_RANGE and enemy.attack_cooldown_timer <= 0.0:
			...
			enemy.transition_to("SoldierAttackState")
			return
		enemy._face_player()
...
# ---------- AttackState ----------
class SoldierAttackState extends State:
	func enter() -> void:
		enemy.has_dealt_damage = false
		enemy.velocity.x = 0.0
		enemy._face_player()
...
func transition_to(target_state_name: String) -> void:
	...
	match target_state_name:
		"SoldierPatrolState":
			current_state = PatrolState.new(self)
		"SoldierChaseState":
			current_state = SoldierChaseState.new(self)
		"SoldierAttackState":
			current_state = SoldierAttackState.new(self)
		"HurtState":
...
func _on_attack_finished() -> void:
	if is_instance_valid(self) and current_state is SoldierAttackState:
		attack_cooldown_timer = ATTACK_COOLDOWN
		transition_to("SoldierChaseState")
